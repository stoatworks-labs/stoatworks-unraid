...
- `unraid.json` — repo visibility, host port assignments, template metadata.
  `repo` is optional there and defaults to the fleet entry's; set it only
  where the image is published from a differently named repo.
...
- The *Run your own copy* section in each browser tool's `README.md`, between
  `<!-- selfhost:start -->` / `<!-- selfhost:end -->`, is generated by
...
directory, and a scratch directory of symlinks named after each repo — each
pointing at that repo's worktree — routes every write there. The shared
checkouts are never touched, and there is nothing to restore afterwards.
...
- **A template with neither `<Support>` nor `<Project>` is dropped from the
  CA feed, silently.** Six went out that way because `unraid.json` did not
  name their repo and the generator emitted both elements empty. It now
  throws instead. There is no error from CA — the app is simply not there.
- **The CA feed scrapes `templates-private/` as well.** A private template is
  listed publicly and points at a package nobody can pull. Keep that
  directory empty; a private package has to go public before its template
